...
class VerifyingPaymasterAPI extends sdk_1.PaymasterAPI {
    ...
    async getPaymasterAndData(userOp) {
        ...
        op.verificationGasLimit = ethers_1.ethers.BigNumber.from(op.verificationGasLimit).mul(3);
        // Ask the paymaster to sign the transaction and return a valid paymasterAndData value.
        const params = [await OptoJSON(op), this.entryPoint, { "type": "payg" }];
        const provider = new ethers_1.ethers.providers.JsonRpcProvider(paymasterUrl);
...
async function main() {
    ...
    const owner = new ethers_1.ethers.Wallet(signingKey);
    const accountAPI = new sdk_1.SimpleAccountAPI({
        provider,
        entryPointAddress,
        owner,
        factoryAddress,
    ...
    const client = new HttpRpcClient_1.HttpRpcClient(rpcUrl, entryPointAddress, chainId);
